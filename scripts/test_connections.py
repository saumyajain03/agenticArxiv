...
async def check_openai(settings):
    ...
    client = AsyncOpenAI(api_key=settings.openai_api_key, base_url=settings.openai_base_url, timeout=10)
    page = await client.models.list()
    gpt_models = [m.id for m in page.data if "gpt" in m.id or "llama" in m.id][:3]
...
async def check_redis(settings):
    ...
    try:
        ...
        version = "unknown (info restricted)"
    client.close()
