...
class ChunkingSettings(BaseConfigSettings):
    ...
    chunk_size: int = 200  # Target words per chunk (child)
    overlap_size: int = 40  # Words to overlap between chunks (child)
    min_chunk_size: int = 40  # Minimum words for a valid chunk
    section_based: bool = True  # Use section-based chunking when available
    parent_chunk_size: int = 1500  # Target words for parent context window
    parent_overlap_size: int = 200  # Words to overlap between parent contexts
...
class PineconeSettings(BaseConfigSettings):
    model_config = SettingsConfigDict(
        env_prefix="PINECONE__",
        extra="ignore",
        frozen=True,
        case_sensitive=False,
    )
