...
class ArxivClient:
    ...
    async def fetch_papers(
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"Fetching {max_results} {self.search_category} papers from arXiv (attempt {attempt + 1}/{max_retries})"
                )
                ...
                if status_code in (429, 503) and attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"arXiv API returned {status_code}. Waiting {wait_time}s before retry {attempt + 2}/{max_retries}..."
                    )
