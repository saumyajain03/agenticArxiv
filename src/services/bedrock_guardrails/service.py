...
class BedrockGuardrailsService:
    ...
    def is_enabled(self) -> bool:
        """Return True if AWS Bedrock Guardrails are configured and active."""
        return bool(self._cfg.guardrail_id)
    ...
    async def check_output(self, answer: str, source_docs: List[str], query: str = "") -> GuardrailResult:
        ...
        content: List[Dict[str, Any]] = [
            {"text": {"text": doc, "qualifiers": ["grounding_source"]}} for doc in source_docs if doc.strip()
        ]
        if query:
    ...
    async def _apply_guardrail(
        ...
        try:
            ...
            if isinstance(e, botocore.exceptions.ClientError):
                ...
                raise BedrockGuardrailsException(f"Bedrock Guardrails API error ({error_code}): {e}")
        ...
        outputs = [block["text"] for block in response.get("outputs", []) if "text" in block]
            ...
            if grounding.get("filters"):
                for f in grounding["filters"]:
                    if f.get("action") == "BLOCKED":
                        ...
                        reasons.append(
                            f"grounding_blocked:{filter_type} score={score:.2f} (threshold={f.get('threshold', 0.7):.2f})"
                        )
