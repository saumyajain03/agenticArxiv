...
class DoclingParser:
    """Docling PDF parser for scientific document processing with a lightweight fallback."""
    ...
    def parse_pdf(self, pdf_path: Path) -> Optional[PdfContent]:
        """Parse PDF using Docling parser (or fallback lightweight parser).
        ...
        try:
            ...
            if self.use_lightweight:
                logger.info(f"Using lightweight fallback parser (pdfium) for {pdf_path.name}")
                return self._parse_with_pdfium(pdf_path)
    ...
    def _parse_with_pdfium(self, pdf_path: Path) -> PdfContent:
        """Lightweight fallback parser using pypdfium2."""
        try:
            doc = pdfium.PdfDocument(str(pdf_path))
            full_text = ""
            sections = []

            # Treat each page as a section to allow semantic chunking
            for i in range(len(doc)):
                ...
                if text:
                    full_text += text + "\n"
                    sections.append(PaperSection(title=f"Page {i + 1}", content=text.strip()))
            ...
            return PdfContent(
                ...
                metadata={"source": "pypdfium2", "note": "Lightweight parser used due to resource constraints"},
            )
        except Exception as e:
            logger.error(f"Fallback pdfium parsing failed for {pdf_path.name}: {e}")
            raise PDFParsingException(f"Fallback pdfium parsing failed: {e}")
