...
class TextChunker:
    ...
    def __init__(
        ...
        parent_overlap_size: int = 200,
    ):
        """Initialize text chunker with parent-child support.

        :param chunk_size: Target number of words per child chunk
        :param overlap_size: Number of overlapping words between child chunks
        :param min_chunk_size: Minimum words for a valid chunk
        :param parent_chunk_size: Target number of words for parent context window
        :param parent_overlap_size: Number of overlapping words between parent windows
        """
        self.chunk_size = chunk_size
        self.overlap_size = overlap_size
        self.min_chunk_size = min_chunk_size
        self.parent_chunk_size = parent_chunk_size
        self.parent_overlap_size = parent_overlap_size

        if overlap_size >= chunk_size:
            raise ValueError("Overlap size must be less than chunk size")
        if parent_overlap_size >= parent_chunk_size:
            raise ValueError("Parent overlap size must be less than parent chunk size")

        logger.info(
            f"Text chunker initialized: chunk_size={chunk_size}, overlap_size={overlap_size}, "
            f"parent_chunk_size={parent_chunk_size}, parent_overlap_size={parent_overlap_size}"
        )
    ...
    def chunk_text(self, text: str, arxiv_id: str, paper_id: str) -> List[TextChunk]:
        """Chunk text into overlapping segments with Parent-Child mapping.
        ...
        if len(words) < self.min_chunk_size:
            ...
            if words:
                reconstructed = self._reconstruct_text(words)
                return [
                    TextChunk(
                        text=reconstructed,
                        ...
                        arxiv_id=arxiv_id,
                        paper_id=paper_id,
                        parent_id=f"{arxiv_id}_parent_0_0",
                        parent_text=reconstructed,
                        document_type="child",
                    )
                ]
            return []
        ...
        while current_position < len(words):
            # Calculate child chunk boundaries
            chunk_start = current_position
            chunk_end = min(current_position + self.chunk_size, len(words))

            # Extract child chunk words
            chunk_words = words[chunk_start:chunk_end]
            chunk_text = self._reconstruct_text(chunk_words)

            # Calculate parent window boundaries (centered around the child chunk)
            center = (chunk_start + chunk_end) // 2
            parent_start = max(0, center - self.parent_chunk_size // 2)
            parent_end = min(len(words), parent_start + self.parent_chunk_size)
            # Adjust start if end hit boundary
            if parent_end == len(words):
                parent_start = max(0, parent_end - self.parent_chunk_size)

            parent_words = words[parent_start:parent_end]
            parent_text = self._reconstruct_text(parent_words)
            parent_id = f"{arxiv_id}_parent_{parent_start}_{parent_end}"
        ...
        logger.info(f"Chunked paper {arxiv_id}: {len(words)} words -> {len(chunks)} Parent-Child chunks")
    ...
    def _create_combined_chunk(
        ...
        if total_words + len(header.split()) < 200 and existing_chunks:
            ...
            existing_chunks[-1] = TextChunk(
                ...
                arxiv_id=arxiv_id,
                paper_id=paper_id,
                parent_id=prev_chunk.parent_id or f"{arxiv_id}_parent_combined_{prev_chunk.metadata.chunk_index}",
                parent_text=merged_text,
                document_type="child",
            )
            return []
    ...
    def _create_section_chunk(
        ...
        parent_id = f"{arxiv_id}_parent_{section_title.replace(' ', '_')}_{chunk_index}"
        return TextChunk(
            text=chunk_text,
            metadata=ChunkMetadata(
                chunk_index=chunk_index,
    ...
    def _split_large_section(
        ...
        enhanced_chunks = []
        parent_id = f"{arxiv_id}_parent_{section_title.replace(' ', '_')}_{base_chunk_index}"
