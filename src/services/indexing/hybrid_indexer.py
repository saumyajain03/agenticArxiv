...
class HybridIndexingService:
    ...
    3. Indexing chunks with embeddings into OpenSearch or Pinecone Cloud
    """
    ...
    def __init__(
        ...
        :param opensearch_client: Optional OpenSearch client
        :param pinecone_client: Optional Pinecone client
        """
    ...
    async def index_paper(self, paper_data: Dict) -> Dict[str, int]:
        ...
        try:
            ...
            # Step 4: Index chunks into OpenSearch or Pinecone
            success_count = 0
            failed_count = 0
            ...
            if self.opensearch_client:
                ...
                pinecone_vectors = []
                for idx, c in enumerate(chunks_with_embeddings):
                    data = c["chunk_data"]
                    vector_id = f"{arxiv_id}_chunk_{data['chunk_index']}"
                    # Prepare metadata dict for Pinecone query retrieval
                    meta = {
                ...
                self.pinecone_client.upsert_vectors(pinecone_vectors)
                success_count = len(pinecone_vectors)
                failed_count = 0
            else:
                logger.error("No indexing backend client configured")
                return {"chunks_created": len(chunks), "chunks_indexed": 0, "embeddings_generated": len(embeddings), "errors": 1}

            logger.info(f"Indexed paper {arxiv_id}: {success_count} chunks successful, {failed_count} failed")
