...
from .nodes import (
    ...
    ainvoke_researcher_step,
    ainvoke_retrieve_step,
    ainvoke_rewrite_query_step,
    ainvoke_supervisor_plan_step,
    ainvoke_writer_step,
...
class AgenticRAGService:
    ...
    def __init__(
        ...
        embeddings_client: Optional[JinaEmbeddingsClient] = None,
        ...
        :param opensearch_client: Optional client for document search
        :param pinecone_client: Optional client for Pinecone search
    ...
    def _build_graph(self):
        ...
        workflow.add_node("supervisor_plan", ainvoke_supervisor_plan_step)
        workflow.add_node("researcher", ainvoke_researcher_step)
        workflow.add_node("writer", ainvoke_writer_step)
        workflow.add_node("critic", ainvoke_critic_step)
        ...
        # Supervisor plan → Researcher
        workflow.add_edge("supervisor_plan", "researcher")

        # Researcher -> Writer -> Critic
        workflow.add_edge("researcher", "writer")
        workflow.add_edge("writer", "critic")

        # Critic -> routes based on peer review decision
        def route_after_critic(state: AgentState):
            decision = state.get("routing_decision", "generate_answer")
        ...
        # Compile graph with checkpoint memory and human-in-the-loop interrupt
        logger.info("Compiling LangGraph workflow with checkpointer and HITL interrupt")
        checkpointer = MemorySaver()
        compiled_graph = workflow.compile(checkpointer=checkpointer, interrupt_after=["supervisor_plan"])
    ...
    async def _run_workflow(self, query: str, model_to_use: str, user_id: str, trace) -> dict:
        ...
        try:
            ...
            config = {"configurable": {"thread_id": f"user_{user_id}_session_{int(time.time())}"}}
            ...
            # Check if the graph is suspended on a HITL interrupt
            state_info = await self.graph.aget_state(config)
            if state_info.next:
                logger.info("Graph is suspended on HITL interrupt. Autoresuming to complete synthesis...")
                # To resume a suspended graph, invoke it with None as the state input
                result = await self.graph.ainvoke(
                    None,
                    config=config,
                    context=runtime_context,
