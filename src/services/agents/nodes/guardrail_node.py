...
async def ainvoke_guardrail_step(
    ...
    try:
        if runtime.context.guardrails_service and runtime.context.guardrails_service.is_enabled:
            ...
            # Use local LLM-based guardrail validation using GUARDRAIL_PROMPT
            from ..prompts import GUARDRAIL_PROMPT
            
            logger.info("No guardrail service configured — falling back to local LLM-based guardrail")
            
            # Create standard LLM model
            ...
            try:
                ...
                score = int(guardrail_data.get("score", 100))
                reason = guardrail_data.get("reason", "Passed")
            except Exception as e:
                logger.warning(f"Failed to parse guardrail JSON response: {e}. Content was: {content}")
                score = 100
                reason = f"Parsing failed, passed by default: {str(e)}"
                
            sanitized_query = None
            logger.info(f"Local guardrail scoring: score={score}, reason={reason}")
