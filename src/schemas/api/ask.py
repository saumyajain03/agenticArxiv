...
class AskRequest(BaseModel):
    ...
    model: Optional[str] = Field(
        None, description="Model ID for generation (provider-specific; omit to use server-configured default)"
    )
...
class AgenticAskResponse(AskResponse):
    ...
    guardrail_filter: Optional[str] = Field(
        None,
        description="Guardrail filter type that acted on this request (e.g. topic_blocked, content_blocked:HATE, pii_blocked:EMAIL, pii_anonymized:PHONE, passed)",
    )
    output_guardrail_filter: Optional[str] = Field(
        None,
        description="Output guardrail result — grounding/relevance check on generated answer (e.g. grounding_blocked:grounding score=0.49, or Content passed all guardrail checks)",
    )
